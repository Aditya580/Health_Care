  ...
  return (
    <div className="min-h-screen flex justify-center items-center relative bg-gradient-to-b from-sky-100 to-blue-100 overflow-hidden p-4">
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-center gap-4">
            {femaleAvatars.map((img) => (
              <img
                key={img}
                src={`/png/images/${img}`} // ✅ corrected path
                alt={img}
                className={`w-14 h-14 rounded-full cursor-pointer border-2 transition-transform duration-300 ease-in-out ${
                  avatar === img
                    ? "border-blue-500 scale-110 shadow-lg"
        ...
        {/* Input Fields */}
        {["First Name", "Last Name", "Email", "Date of Birth"].map(
          (placeholder, idx) => {
            const type =
              placeholder === "Email"
                ? "email"
      ...
      <svg
        className="absolute top-10 left-5 w-16 h-16 text-blue-200 opacity-30 animate-pulse"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
      ...
      <svg
        className="absolute bottom-10 right-5 w-20 h-20 text-sky-200 opacity-25 animate-pulse"
        viewBox="0 0 24 24"
        fill="currentColor"
      >
        <circle cx="12" cy="12" r="12" />
